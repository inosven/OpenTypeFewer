...
class VoicePadApp:
    def __init__(self, config_path: str = None, verbose_mode: bool = False):
        ...
        logger.info("VoicePad starting up")
    ...
    def run(self) -> None:
        ...
        logger.info("VoicePad running (main loop started)")
    ...
    def open_settings(self) -> None:
        if self.tk_root and self.settings_gui:
    ...
    def shutdown(self) -> None:
        logger.info("VoicePad shutting down")
        self.hotkey_manager.stop_listening()
        self.tray_app.quit_tray()
        if self.tk_root:
            self.tk_root.after(0, self.tk_root.destroy)

    def _process_audio_pipeline(self, audio_file_path: str) -> None:
        try:
